...
// Sphereを表示する疑似コード
void DrawSphere(const Pendulum& pendulum, const Matrix4x4& viewProjectionMatrix, const Matrix4x4& viewportMatrix)
{
	float pi = float(M_PI);
	...
	for (uint32_t latIndex = 0; latIndex < kSubdivision; ++latIndex)
	{
		...
		for (uint32_t lonIndex = 0; lonIndex < kSubdivision; ++lonIndex)
		{
			...
			// ワールド座標系での頂点を求める
			a = {
				(pendulum.radius) * std::cos(lat) * std::cos(lon) + pendulum.position.x,
				(pendulum.radius) * std::sin(lat) + pendulum.position.y,
				(pendulum.radius) * std::cos(lat) * std::sin(lon) + pendulum.position.z
			};

			b = {
				(pendulum.radius) * std::cos(lat + kLatEvery) * std::cos(lon) + pendulum.position.x,
				(pendulum.radius) * std::sin(lat + kLatEvery) + pendulum.position.y,
				(pendulum.radius) * std::cos(lat + kLatEvery) * std::sin(lon) + pendulum.position.z
			};

			c = {
				(pendulum.radius) * std::cos(lat) * std::cos(lon + kLonEvery) + pendulum.position.x,
				(pendulum.radius) * std::sin(lat) + pendulum.position.y,
				(pendulum.radius) * std::cos(lat) * std::sin(lon + kLonEvery) + pendulum.position.z
			};
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		// 線の座標変換
		// 始点
		Vector3 ndcVertexAnchor = Transform(pendulum.anchor, worldViewProjectionMatrix);
